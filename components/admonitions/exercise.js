...
export default function Exercise(props) {
  ...
  return (
    <div className="my-12 rounded border border-gray-700 p-4">
      {props.children.length ? (
        <>
          {props.children.filter(
            (child) => child.props.mdxType === 'Question' || child.type.name === 'Question'
          )}

          <div className={hidden ? 'hidden' : ''}>
            {props.children.filter(
              (child) => child.props.mdxType === 'Answer' || child.type.name === 'Answer'
            )}
          </div>
        </>
      ) : (
